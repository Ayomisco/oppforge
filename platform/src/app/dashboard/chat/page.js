                  ...
                  {msg.role === 'user' && (
                    <div className="absolute top-0 -left-12 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-1">
                      <button 
                        ...
                        className="p-1.5 bg-[#1a1512] border border-white/5 rounded hover:border-[#ff5500] hover:text-[#ff5500] transition-all"
                      >
                        <Edit3 size={12} />
                      </button>
                      <button 
                        onClick={() => handleDelete(msg.id)}
                        className="p-1.5 bg-[#1a1512] border border-white/5 rounded hover:border-red-500 hover:text-red-500 transition-all"
                      >
                        <Trash2 size={12} />
                      </button>
                    </div>
      ...
      <div className="p-4 bg-[var(--bg-espresso)] border-t border-[var(--glass-border)]">
        <form onSubmit={handleSend} className="relative group/form">
          ...
          <button 
            type="submit"
            disabled={!input.trim()}
            className="absolute right-2 top-1/2 -translate-y-1/2 p-2 text-[#ff5500] hover:bg-[#ff5500] hover:text-black rounded transition-all disabled:opacity-30 disabled:cursor-not-allowed group-focus-within/form:shadow-[0_0_10px_#ff5500]"
          >
            <Send size={18} />
          </button>
        </form>

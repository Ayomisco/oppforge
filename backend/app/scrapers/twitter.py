...
class TwitterScraper(BaseScraper):
    ...
    def fetch(self) -> List[Dict[str, Any]]:
        ...
        if not results:
            print(
                f"[{self.source_name}] RapidAPI failed or empty. Trying Nitter fallback...")
            ...
            for query in queries[:5]:  # Limit to 5 queries per run for now
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        # We'll try to adapt.
                        tweets = data.get("results", []) or data.get(
                            "timeline", [])
                        for t in tweets:
                            t["_query"] = query
                            raw_data.append(t)
                        print(
                            f"[{self.source_name}] RapidAPI for '{query}': Found {len(tweets)} tweets.")
                    else:
                        print(
                            f"[{self.source_name}] RapidAPI Error {response.status_code}: {response.text[:100]}")
                    ...
                    try:
                        ...
                        if res.status_code == 200:
                            ...
                            for t in tweets:
                                ...
                                tweet_id = href.split(
                                    "/")[-1].replace("#m", "") if href else str(random.randint(1000, 9999))
                            ...
                            print(
                                f"[{self.source_name}] Nitter ({instance}) for '{query}': Found {len(tweets)} tweets.")
                            success = True
                        else:
                            print(
                                f"[{self.source_name}] Nitter ({instance}) failed: {res.status_code}")

                    except Exception as e:
                        print(
                            f"[{self.source_name}] Nitter ({instance}) error: {e}")
    ...
    def parse(self, raw_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for tweet in raw_data:
            ...
            if "hackathon" in lower_text:
                found_category = "Hackathon"
            elif "bounty" in lower_text:
                found_category = "Bounty"
            elif "airdrop" in lower_text:
                found_category = "Airdrop"
            elif "testnet" in lower_text:
                found_category = "Testnet"

            # Filter non-actionable chatter
            action_keywords = ["apply", "register", "live",
                               "portal", "grant", "submit", "testnet", "incentivized"]

...
import { ArrowRight, Terminal, Zap, Shield, Search, Menu, X, Rocket, ChevronRight } from 'lucide-react'

// --- Simple Components (inline for landing page speed) ---
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            className="md:hidden border-b border-[var(--border-subtle)] bg-[var(--bg-espresso)] overflow-hidden"
          >
...
const FeatureCard = ({ icon: Icon, title, description, delay }) => (
  <motion.div 
    ...
    viewport={{ once: true }}
    className="glass-card p-6 flex flex-col gap-4 group hover:bg-[var(--bg-walnut)] transition-colors"
  >
        ...
        <div className="container">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <FeatureCard 
              ...
              description="AI agents scan Twitter, Discord, and governance forums 24/7 to find hidden alpha before it goes viral."
              delay={0.1}
            />
            <FeatureCard 
              icon={Zap}
              title="Real-Time Scoring"
              description="Every opportunity is scored (0-100) based on your specific skills, preferred chains, and historical win probability."
              delay={0.2}
            />
            <FeatureCard 
              icon={Terminal}
              title="Forge AI Chat"
              description="Ask specific questions like 'Is this grant worth my time?' or 'Draft a proposal for this hackathon'."
              delay={0.3}
            />
          </div>
        </div>
            ...
            <div className="lg:w-1/2">
              ...
              <div className="space-y-6">
                <div className="flex gap-4">
                  <div className="w-12 h-12 rounded bg-[var(--bg-mahogany)] flex items-center justify-center shrink-0 text-[var(--accent-forge)]">
                    1
                  </div>
                  <div>
                    <h3 className="text-xl font-bold mb-2">Connect Your Profile</h3>
                    <p className="text-[var(--text-secondary)]">Tell OppForge your skills (Rust, Solidity, Design) and preferred chains.</p>
                  </div>
                </div>
                <div className="flex gap-4">
                  <div className="w-12 h-12 rounded bg-[var(--bg-mahogany)] flex items-center justify-center shrink-0 text-[var(--accent-forge)]">
                  ...
                  <div>
                    <h3 className="text-xl font-bold mb-2">Receive Curated Alpha</h3>
                    <p className="text-[var(--text-secondary)]">Get a personalized feed of high-probability opportunities, filtered by AI.</p>
                  </div>
                </div>
                <div className="flex gap-4">
                  <div className="w-12 h-12 rounded bg-[var(--bg-mahogany)] flex items-center justify-center shrink-0 text-[var(--accent-forge)]">
                  ...
                  <div>
                    <h3 className="text-xl font-bold mb-2">Execute & Win</h3>
                    <p className="text-[var(--text-secondary)]">Use the AI assistant to draft proposals and track your applications.</p>
                  </div>
                </div>
              </div>
            </div>
